...
function iframeSized(iframe: HTMLIFrameElement): boolean {
  const rect = iframe.getBoundingClientRect();
  return rect.width >= MIN_IFRAME_SIZE && rect.height >= MIN_IFRAME_SIZE;
}

function frameLooksRendered(iframe: HTMLIFrameElement): boolean {
  try {
    ...
    // cross-origin — fall through to size check
  }
  return iframeSized(iframe);
}

function countRendered(iframes: HTMLIFrameElement[]): number {
  return iframes.filter(frameLooksRendered).length;
}

export function setupFacebook(): void {
  const wrapper = document.getElementById('facebook-embeds');
  if (!wrapper) return;

  const iframes = Array.from(
    wrapper.querySelectorAll<HTMLIFrameElement>('iframe[src*="facebook.com/plugins"]'),
  );
  ...
  whenPlatformReady(wrapper, () => {
    ...
    const showFallback = () => {
      ...
      settled = true;
      clearWatchers();
      toggleGroupFallback('fb-fallback', 'facebook-embeds', true);
    };
    ...
    const showEmbeds = () => {
      ...
      settled = true;
      clearWatchers();
      toggleGroupFallback('fb-fallback', 'facebook-embeds', false);
    };
